...
# Name : Calister Apollonary Simba
# ID   : R00224887

# ATTACK AUTOMATION TOOL

# This tool is used to automate the process of discovering weak usernames and passwords  
...
from ftplib import FTP #This was added for file deployment in the part 8 of the assignment
...
# The function should open the file and read the contents of the file into a list, where each element in list is an IP address 
...
def read_ip_list(ip_file):
    ...
    try:
        with open(ip_file) as f:
            ...
            for line in f: 
                ...
                # I used another source here to gain more understanding and improve my scripts
    ...
    # This will checking to see if we have atleast one ip address in the list
    if len(ip_list) < 1:
...
# TCP is a three-way handshake however sending TCP SYN scan doesn't complete the handshake
...
def scan_port(ip, port):
    # Here I use scapy to create an IP packet with a destination address of the given IP
    # Then I created TCP header containing the destination port of the given port, and 
    # set the TCP flags to be the appropriate flags for a SYN scan ("S")
    
    p = IP(dst=ip)/TCP(dport=port, flags='S')  
    resp= sr1(p, timeout=1, verbose=0)  # Using sr1 function to send and receive packets
    try:
...
# This function will be used to attempt to  bruteforce the Telnet username and password for a given host.
# This function  takes in an IP address, a port number, a username which is provided by the user using -u, 
# and a password filename provided by the user using -f option.
...
def bruteforce_telnet(ip, port, username, password_list_filename):
    ...
    for i, passwd in enumerate(passwd_list):
        ...
        try:
            ...
            # I faced a challenge here in which I had to refer to other resources for clarification
            # Reference : https://stackoverflow.com/questions/47087138/python3-telnet-read-all-doesnt-work
...
# This function will be used to attempt to  bruteforce the SSH username and password for a given host
# Here I use python paramiko library defined in line 22 to create SSH connections
...
def bruteforce_ssh(ip, port, username, password_list_filename):
    ...
    for i, passwd in enumerate(passwd_list):
        ...
        try:
            ...
            # Trying to open a session -- checking if we are successfuly logged in
...
def deploy_telnet(ip, port,username, password, file_name):
    ...
    if port_21:  # if port 21 is open, connect to the FTP server and upload file
        ...
        if port_22:  # if port 22 is open, connect via SSH and upload file to the user's home directory
            try:
                deploy_ssh(ip, port_22, username, password, file_name)
            except:
                print(f'Connection problem, the file could\'nt be uploaded to {remote_path}')
                return
...
# Please note that in order for me to see clearly how my scripts work and arrange properly
# I had to put some sleep functions as well as printing out some instructions
# This will enable even a user running these scripts to get understanding of how this works!
# start the program
def main():
